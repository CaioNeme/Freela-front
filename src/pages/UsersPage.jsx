...
export default function Users() {
  ...
  useEffect(() => {
    ...
    const services = axios.get(`${import.meta.env.VITE_API_URL}/users`, config)
    services.then(services => {
      setServices(services.data.services);
    })
    const contracts = axios.get(`${import.meta.env.VITE_API_URL}/contract`, config)
            ...
            <li key={contract.id}>
              ...
              <div>
                <button onClick={() => {
                  ...
                  const promisse = axios.put(`${import.meta.env.VITE_API_URL}/contract/${contract.id}`, obj, config)
                  promisse.then(() => {
              ...
              <div>
                {service.status === "true" ?
                  <button onClick={() => {
                    ...
                    const promisse = axios.put(`${import.meta.env.VITE_API_URL}/service/${service.id}`, obj, config)
                    promisse.then(() => {
                      setRefresh(refresh + 1)
                    })
                    promisse.catch(erro => {
